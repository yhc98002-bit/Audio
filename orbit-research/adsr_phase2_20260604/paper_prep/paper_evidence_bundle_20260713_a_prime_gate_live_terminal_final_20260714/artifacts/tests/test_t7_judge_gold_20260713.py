...
def test_gpu_queue_is_nonpreemptive_four_gpu_same_node_and_signature_gated():
    ...
    assert module.signature_ready() is True
...
def test_w2_signature_verification_records_both_pis_and_separates_adoption():
    ...
    assert "Name: Richard" in amendment
    assert "Date: 2026-07-13" in amendment
    assert amendment.count("Commit SHA: 0df1cbb") == 2
    assert "PI 2 provenance: pi:GPT-5.6-Pro" in amendment
    assert "Auditing co-PI (Claude)" in amendment
    assert "W2_AMENDMENT_STATUS = SIGNED_BY_BOTH_PIS" in report
    assert "W2_ADOPTION = PI1_SIGNED_PI2_INCOMPLETE" in report
    assert "LIVE_CONFIRM_STATUS = COMPLETE_CRITERIA_NOT_ALL_MET" in report
