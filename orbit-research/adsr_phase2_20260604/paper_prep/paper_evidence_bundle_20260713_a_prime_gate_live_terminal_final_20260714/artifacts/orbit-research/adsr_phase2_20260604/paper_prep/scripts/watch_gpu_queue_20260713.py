...
JUDGE_MODEL = "/dev/shm/ADSR_QWEN3_OMNI_MODELS/Qwen3-Omni-30B-A3B-Instruct"
JUDGE_ENV = "/dev/shm/adsr_qwen_omni_env"
...
def signature_ready() -> bool:
    ...
    return (
        ...
        and "PI 1 provenance: pi:Richard" in amendment
        and "PI 2 provenance: pi:GPT-5.6-Pro" in amendment
        and "Auditing co-PI (Claude)" in amendment
    )


def judge_runtime_ready(node: str) -> bool:
    command = (
        f"test -f {JUDGE_MODEL}/model.safetensors.index.json && "
        f"test -f {JUDGE_MODEL}/STAGING_COMPLETE && "
        f"test -x {JUDGE_ENV}/bin/vllm && "
        f"! find {JUDGE_MODEL} -maxdepth 1 -type f -name '*.incomplete' -print -quit | grep -q ."
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--nodes",
        nargs="+",
        help="override eligible nodes; judge defaults to an29 and live defaults to an12/an29",
    )
    args = parser.parse_args()
    required = 4
    default_nodes = ["an29"] if args.job == "judge" else ["an12", "an29"]
    ...
    while True:
        ...
        for node in nodes:
            try:
                ...
                runtime_ready = judge_runtime_ready(node) if args.job == "judge" else True
                observations.append(
                    {
                        "node": node,
                        "state": state,
                ...
                if len(free) >= required and signatures and runtime_ready:
        ...
        record = {
            ...
            "outputs": [str(log)],
            "status": "WAITING",
            "next_action": f"poll again in {args.poll_seconds} seconds without preemption",
        ...
        for node, indices in candidates:
            ...
            launcher_log = (
                PAPER
                / "w2_execution_20260712/live_confirmation_20260713/logs/launcher.stdout.log"
            )
            launch_command = (
                f"bash {shlex.quote(str(script))} {shlex.quote(gpu_list)} "
                f">> {shlex.quote(str(launcher_log))} 2>&1"
            )
            remote = (
                f"cd {shlex.quote(str(ROOT))} && "
                f"(tmux has-session -t {shlex.quote(remote_session)} 2>/dev/null || "
                f"tmux new-session -d -s {shlex.quote(remote_session)} "
                f"{shlex.quote(launch_command)})"
            )
        ...
        if time.time() - started >= args.timeout_seconds:
            ...
            (out / f"{args.job.upper()}_GPU_QUEUE_24H_ESCALATION.md").write_text(
                ...
                + (
                    f"The judge requires four idle GPUs on one staged-runtime node ({', '.join(nodes)}); its TP=4 service cannot split across nodes. "
                    if args.job == "judge"
                    else "The prepared live launcher requires four idle GPUs on one node; a reduced-prompt run would still require one GPU but would deviate from the frozen 64-prompt design. "
                )
