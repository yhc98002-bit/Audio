...
python paper_prep/scripts/preflight_w2_reward_models_20260714.py \
  >"${OUT}/OFFLINE_REWARD_PREFLIGHT_${RUN_TAG}.json"
...
if [[ ! -s "${OUT}/ACTUAL_LAUNCH_TIMESTAMP.txt" ]]; then
  date -Iseconds >"${OUT}/ACTUAL_LAUNCH_TIMESTAMP.txt"
fi
if [[ ! -s "${OUT}/HARD_STOP_DEADLINE.txt" ]]; then
  date -d '+48 hours' -Iseconds >"${OUT}/HARD_STOP_DEADLINE.txt"
fi
deadline=$(date -d "$(cat "${OUT}/HARD_STOP_DEADLINE.txt")" +%s)
if (( $(date +%s) >= deadline )); then
  echo "LIVE_CONFIRM_STATUS = CAP_MISS" >"${OUT}/LIVE_CONFIRM_TERMINAL_STATUS.txt"
  exit 124
fi
printf '%s\n' "$(date -Iseconds)" >"${OUT}/RESUME_LAUNCH_${RUN_TAG}.txt"
echo "LIVE_CONFIRM_STATUS = RUNNING" >"${OUT}/LIVE_CONFIRM_TERMINAL_STATUS.txt"
pids=()
for worker in 0 1 2 3; do
  ...
  python paper_prep/scripts/w2_liveconfirm_worker_20260713.py \
    ...
    >"${OUT}/logs/worker_${worker}_${RUN_TAG}.stdout.log" \
    2>"${OUT}/logs/worker_${worker}_${RUN_TAG}.stderr.log" &
  pids+=("$!")
done
