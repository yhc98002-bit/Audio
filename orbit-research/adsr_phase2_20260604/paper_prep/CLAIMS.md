...
| 4 | An early detector + condition-changing restart wins at equal compute in a live system, with no quality cost. | Live 8-group controlled experiment, 22,825 generation attempts: violations 0.98% vs 11.5% at equal compute; more usable outputs (+30%); all automatic quality scores equal or better. | (a) Human listening test → **T1.3**. (b) Original-request fidelity check → **T2.4**. (c) Honest efficiency numbers → **T2.1** | `PH2/batch3/ADSR_ONLINE_COMPREHENSIVE_RESULTS.md` (+ `.json`; blinded twin `_BLINDED.json`); ledgers `PH2/batch3/online_run/ledger_w*.jsonl` (w0–w15); cost: `PH2/batch3/ADSR_ONLINE_COST_ACCOUNTING.md`, `PH2/batch3/COST_RECONCILIATION.json`; integrity: `PH2/GATE_B_FINAL_REPORT.md` §3 | Primary (pre-reg): restart2+ per-draw clean rate arm6−arm4 **+0.4299** CI95 [0.273, 0.5788] vs criterion ≥+0.15 (power 0.87); by direction: vocal +0.761, instrumental −0.0032. Secondary E2a **+0.3829** CI95 [0.2466, 0.5222]. Selected type-error: arm6 **0.0098** vs arm1 **0.1146**; arm7 (1.43× compute) **0.066**; arm4 (seed-only) 0.123 ≈ arm1. Clean yield @168: **3.7108 vs 2.8614** (+29.7%). Proxy margins all positive: Δcommon +0.0178, Δsemantic +0.0003, Δaesthetic +0.1043, Δlyric(EN-vocal) +0.0307. Ledger: **22,825 attempts / 3,648 units / 0 validation errors** | YHC (pending: T1.3 ____, T2.4 ____, T2.1 ____) |
