...
4. **Baseline:** off-the-shelf (clean-audio-trained) vocal detector applied
   to the early estimate, to test whether the early-σ domain is OOD (which, if
   confirmed, would warrant a learned early detector).
...
6. **Closed loop (offline):** test whether type-match restart improves the
