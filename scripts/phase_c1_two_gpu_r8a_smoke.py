"""Isolated two-GPU rollout-parallel smoke for Phase C1 R8a.
...
def _assert_visible_physical_gpus(expected: tuple[str, ...] = ("5", "6")) -> None:
...
def _assert_target_gpus_idle() -> None:
    ...
    busy = {idx: apps.get(idx, []) for idx in ("5", "6") if apps.get(idx)}
    if busy:
        raise RuntimeError(f"physical GPUs 5/6 are not idle before launch: {busy}")
...
def _worker_main(
    ...
    *,
    ...
    try:
        ...
        method_spec = cfg["methods"][METHOD]
        if method_spec["reward_mode"] != "terminal":
            raise RuntimeError("two-GPU smoke currently supports terminal R8a only")
        ...
        worker_dir = Path(output_dir) / "worker_gpu6"
...
def _write_summary(path: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        "# Phase C1 Two-GPU R8a Smoke Summary",
        "",
...
def run_two_gpu_smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    infer_steps = int(sampler["terminal_infer_step"])
    ...
    result = {
        ...
        "sampler_extras": sampler_extras,
        "rollout_split_policy": "even members on controller GPU5; odd members on worker GPU6",
